...
Screen::Screen(QWidget *parent) : QWidget(parent)
{
    ...
    m_SpectrumScreen.Initialize(BruceLee_scr, bcmFrameBuffer);
}

QSize Screen::minimumSizeHint() const
{
    return QSize(2 * 352 /* 48 + 256 + 48 */, 296 /*(48 + 192 + 56)*/);
}
...
void Screen::paintEvent(QPaintEvent * /* event */)
{
    ...
    painter.drawImage(QPoint(48,48), zxSpectrumImage);
    ...
    for (int i = 0; i < 272; i++) {
        for (int j = 0; j < 176; j++) {
            ...
            raspberryPiImage.setPixel( (j * 2) , i, bcmFrameBuffer->palette[(byte & 0xF0) >> 4] );
            raspberryPiImage.setPixel( (j * 2) + 1 , i, bcmFrameBuffer->palette[byte & 0x0F] );
        }
    }
#endif
     ...
     */
    painter.drawImage(QPoint(352,0), raspberryPiImage);
