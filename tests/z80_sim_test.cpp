...
Z80SimTest::Z80SimTest() : cpu(*this) {
}
...
uint8_t Z80SimTest::fetchOpcodeImpl(uint16_t address) {
    ...
    return (cpmMode && address == 0x0005 ? breakpoint(address, opcode) : opcode);
#endif
}
...
void Z80SimTest::poke8Impl(uint16_t address, uint8_t value) {
...
uint16_t Z80SimTest::peek16Impl(uint16_t address) {
    ...
    uint8_t lsb = peek8Impl(address);
    uint8_t msb = peek8Impl(address + 1);
    return (msb << 8) | lsb;
}

void Z80SimTest::poke16Impl(uint16_t address, RegisterPair word) {
    // Order matters, first write lsb, then write msb, don't "optimize"
    poke8Impl(address, word.byte8.lo);
    poke8Impl(address + 1, word.byte8.hi);
}
...
void Z80SimTest::outPortImpl(uint16_t port, uint8_t value) {
    // 4 clocks for write byte to bus
    tstates += 4;
    z80Ports.at(port) = value;
}

void Z80SimTest::addressOnBusImpl(uint16_t address, int32_t tstates) {
    // Additional clocks to be added on some instructions
    this->tstates += tstates;
}

void Z80SimTest::interruptHandlingTimeImpl(int32_t tstates) {
    this->tstates += tstates;
}

bool Z80SimTest::isActiveINTImpl() {
    // Put here the needed logic to trigger an INT
    return false;
}

#ifdef WITH_EXEC_DONE
void Z80SimTest::execDoneImpl(void) {
}
#endif

#ifdef WITH_BREAKPOINT_SUPPORT
uint8_t Z80SimTest::breakpointImpl(uint16_t address, uint8_t opcode) {
